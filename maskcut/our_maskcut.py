...
INFO = {
    "description": "PASCAL VOC12: pseudo-masks with MaskCut",
    "url": "https://github.com/facebookresearch/CutLER",
    "version": "1.0",
    "year": 2024,
    "contributor": "Kainmueller Lab",
...
if __name__ == "__main__":
    ...
    parser.add_argument('--image_set', type=str, default="train", choices=['train','eval','test'])
    # additional arguments
    parser.add_argument('--dataset-path', type=str, default="/home/xiaoyan/Documents/Data/VOC2012", help='path to the dataset')
    ...
    image_ids = open(os.path.join(args.dataset_path, "ImageSets/Main", f"{args.image_set}.txt")).read().strip().split()
    pbar = tqdm(total=len(image_ids), desc=f'Converting VOC12_{args.image_set}')
    print(f"In total, there are {len(image_ids)} images in {args.image_set} set for object detection.")
    
    for img_name in image_ids:
        img_path = os.path.join(args.dataset_path, "JPEGImages", f"{img_name}.jpg")
        try:
        ...
        I = Image.open(img_path).convert('RGB')
        width, height = I.size
        print(f"This image contains {len(bipartitions)} bipartitions.")
        for idx, bipartition in enumerate(bipartitions):
            ...
            if annotation_info is not None:
                ...
                segmentation_id += 1
                print("--------------------------------")
                print(f"img_name:{img_name}, idx={idx} bipartitions with the following annotation info:")
                print(annotation_info)
        image_id += 1

    # save the whole json name
    json_name = '{}/pascal_voc12_train_fixsize{}_tau{}_N{}.json'.format(args.out_dir, args.fixed_size, args.tau, args.N)

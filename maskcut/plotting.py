...
def get_affinity_matrix(feats, act=set_min):
    ...
    A = act(A) + construct_distant_penalty_matrix(np.sqrt(A.shape[0]), sigma=4)
    d_i = np.sum(A, axis=1)
    D = np.diag(d_i)
...
def main():
    ...
    for (names, act) in [['set_min', set_min], ['softmax', softmax], ['softmax_3T', softmax_3T],
        ...
        for i in range(1, 9):
            ...
            plt.savefig('./Results/res_{}_{}'.format(names, input_image))
